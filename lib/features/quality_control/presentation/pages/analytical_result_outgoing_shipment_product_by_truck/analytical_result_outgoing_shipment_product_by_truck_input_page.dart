...
class _AnalyticalResultOutgoingShipmentProductByTruckInputPageState
  ...
  AppBar _buildAppBar() {
    ...
    return AppBar(
      ...
      title: Text(
        'Analytical Result of Out Going Shipment Product By Truck Input ${formData!.code}',
        style: TextStyle(
      ...
      final detail =
          detailControllers.map((row) {
            return AnalyticalResultOutgoingShipmentProductByTruckDetailEntity(
              ...
              // PERBAIKAN DI SINI: Tambahkan .text sebelum diparsing
              ffa: parseDouble(row['ffa']?.text) ?? 0,
              mni: parseDouble(row['mni']?.text) ?? 0,
              iv: parseDouble(row['iv']?.text) ?? 0,
              lovibondColorRed:
                  parseDouble(row['lovibond_color_red']?.text) ?? 0,
              lovibondColorYellow:
                  parseDouble(row['lovibond_color_yellow']?.text) ?? 0,
              pv: parseDouble(row['pv']?.text) ?? 0,
