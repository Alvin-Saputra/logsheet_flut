    ...
    try {
      ...
      if (AppRoles.leadQC.contains(role)) {
        query = """
          ...
          *
      FROM 
          t_analytical_result_incoming_material_by_vessel h
      INNER JOIN 
          t_analytical_result_incoming_material_by_vessel_detail d 
          ON h.id = d.id_hdr
      WHERE 
      ...
      log(
        'Fetched ${result.rows.length} Analytical Result Incoming Material By Vessel for date $date.',
      );
      return result.rows.map((row) => row.assoc()).toList();
    } catch (e) {
      log(
        'Error fetching all Analytical Result Incoming Material By Vessel based on date: $e',
      );
      return [];
    } finally {
      try {
